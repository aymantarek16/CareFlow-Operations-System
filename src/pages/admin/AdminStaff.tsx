...
export default function AdminStaff() {
  ...
  const [editForm, setEditForm] = useState<{ name: string; role: StaffRole }>({
    name: "",
    role: "receptionist",
  });
              ...
              action={
                search ? undefined : (
                  <p className="text-xs text-foreground/50">
                    أضف أول موظف من الفورم بجانب الجدول
                  </p>
                )
              }
            />
          ...
          {loading ? (
            ...
            <form onSubmit={handleCreate} className="grid gap-3">
              ...
              <p className="rounded-xl border border-amber-400/20 bg-amber-400/5 p-3 text-[11px] leading-5 text-amber-200/80">
                ملاحظة: حذف الموظف يزيل صلاحياته من النظام فقط. لحذف حساب
                المصادقة نهائياً اذهب إلى Supabase Dashboard →
